...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl shadow-xl p-8 md:p-12 mb-8 text-white">
        <div class="max-w-3xl">
            ...
            <div class="flex flex-wrap gap-4">
                <a href="#events" class="inline-flex items-center px-6 py-3 bg-white text-indigo-600 font-semibold rounded-lg hover:bg-indigo-50 transition">
                    Jelajah Event
                ...
                @auth
                    @if(auth()->user()->isOrganizer())
                        <a href="{{ route('events.create') }}" class="inline-flex items-center px-6 py-3 bg-indigo-500 text-white font-semibold rounded-lg hover:bg-indigo-400 transition border-2 border-white">
                            Buat Event
                        </a>
                    @endif
                @endauth
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-lg shadow p-6 text-center">
            <div class="text-3xl font-bold text-indigo-600 mb-2">{{ \App\Models\Event::published()->count() }}</div>
            <div class="text-gray-600">Event Aktif</div>
        </div>
        <div class="bg-white rounded-lg shadow p-6 text-center">
            <div class="text-3xl font-bold text-green-600 mb-2">{{ \App\Models\User::organizers()->count() }}</div>
            <div class="text-gray-600">Organizer</div>
        </div>
        <div class="bg-white rounded-lg shadow p-6 text-center">
            <div class="text-3xl font-bold text-purple-600 mb-2">{{ \App\Models\Registration::confirmed()->count() }}</div>
            <div class="text-gray-600">Peserta</div>
        </div>
        <div class="bg-white rounded-lg shadow p-6 text-center">
            <div class="text-3xl font-bold text-amber-600 mb-2">{{ \App\Models\Certificate::count() }}</div>
            <div class="text-gray-600">Sertifikat</div>
        </div>
    </div>
